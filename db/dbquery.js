...
module.exports.getclassfriend = function (data, callback) {
    db_init.reserve(function (connObj) {
        var conn = connObj.conn;
        conn.createStatement(function (err, statement) {
            if (err) {
                ...
                var query = "SELECT * FROM users u JOIN" +
                    " user_class uc on u.userid = uc.userid " +
                    "WHERE uc.classid='" + data + "'";
                console.log(query);
                statement.executeQuery(query,
                    function (err, resultset) {
                        if (err) {
...
module.exports.getgroupfriend = function (data, callback) {
    db_init.reserve(function (connObj) {
        var conn = connObj.conn;
        conn.createStatement(function (err, statement) {
            if (err) {
